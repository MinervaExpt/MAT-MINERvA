...
#ifndef GenericVerticalSystematics_h
#define GenericVerticalSystematics_h
...
namespace PlotUtils
{
  ...
  class GenericVerticalUniverse: public BASE_UNIVERSE
  {
    ...
    GenericVerticalUniverse(typename BASE_UNIVERSE::config_t& config, std::unique_ptr<PlotUtils::Reweighter<BASE_UNIVERSE, EVENT>>&& weighter, double scale = 1.0): BASE_UNIVERSE(config), fReweight(std::move(weighter)), fScaleAboutCV(scale)
      ...
      double GetGenericWeight() const override
      {
        ...
        double cv_wgt = BASE_UNIVERSE::GetGenericWeight();

        //return cv_wgt*fScaleAboutCV*fReweight->GetWeight(*this,empty);
        ...
        else if (fScaleAboutCV > 0) return fScaleAboutCV*fReweight->GetWeight(*this, empty)*cv_wgt;
        else return (2+fScaleAboutCV*fReweight->GetWeight(*this,empty)*cv_wgt );//This is to be used as a reflection about the CV of the "upshift universe" for the case of a positive factor. - David L. July 29, 2022  
      ...
      double GetWeightRatioToCV() const override
      {
        ...
        else return (2+fScaleAboutCV*fReweight->GetWeight(*this,empty) );//This is to be used as a reflection about the CV of the "upshift universe" for the case of a positive factor. - David L. July 29, 2022      
      }
...
#endif // GenericVerticalSystematics_h